...
calm_behavior_enabled = True
attack_behavior_enabled = False
evade_behavior_enabled = False
...
class Boid:
    ...
    Returns:
     ...
     """
        ...
        if distance < evade_distance and distance > 0:  # Added check for distance > 0
            evade_force = evade_strength / max(distance, 0.1)  # Use max to avoid division by zero, 0.1 is an arbitrary small number
            return multiply_vector(to_target, evade_force)
        return [0, 0]
    ...
    def apply_calm_behavior(self, neighbors):
        ...
        self.sprite.vel = add_vector(self.sprite.vel, align) # actualizarea vitezei sprite-ului, adÄugĂ˘nd la vectorul sÄu de vitezÄ curent un alt vector,
...
# Event handlers
def toggle_calm_behavior():
    global calm_behavior_enabled
    calm_behavior_enabled = not calm_behavior_enabled

def toggle_attack_behavior():
    global attack_behavior_enabled
    attack_behavior_enabled = not attack_behavior_enabled

def toggle_evade_behavior():
    global evade_behavior_enabled
    evade_behavior_enabled = not evade_behavior_enabled
...
def keyup(key):
    ...
    if key == simplegui.KEY_MAP["left"] or key == simplegui.KEY_MAP["right"]:
        my_ship.turn(0)  # Stop turning when left or right key is released
    elif key == simplegui.KEY_MAP["up"]:
        my_ship.accelerate(0)  # Stop thrust when the up key is released
    elif key == simplegui.KEY_MAP["space"]:
        # Optionally, handle missile firing logic if it's designed to fire continuously while holding down
        pass
...
inputs = {"left": (my_ship.turn, -1),
          ...
          "a": (toggle_calm_behavior, None),
          "b": (toggle_attack_behavior, None),
          "c": (toggle_evade_behavior, None)
          }
