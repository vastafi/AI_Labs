...
def backward_chain(rules, hypothesis, verbose=False ) :
    ...
    while new_antecedents_added:
        ...
        for rule in rules:
            ...
            if consequent:
                consequent_str = str(consequent).split(' ', 1)[1].rstrip("')")
                if consequent_str == hypothesis_str:
                    antecedent = rule.antecedent()
                    if consequent_str not in results:
                        results[consequent_str] = []
                        new_antecedents_added = True
                    if antecedent and antecedent not in results[consequent_str]:
                        results[consequent_str].append(antecedent)
                        new_antecedents_added = True
